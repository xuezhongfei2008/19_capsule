...
class CapsLayer(object):
    ...
    Args:
        input: A 4-D tensor.
        num_outputs: the number of capsule in this layer.	胶囊个数
        vec_len: integer, the length of the output vector of a capsule.	胶囊特征维度
    ...
    def __call__(self, input, kernel_size=None, stride=None):
        ...
        if self.layer_type == 'FC':
            if self.with_routing:
                ...
                print("self.input1", input)
                self.input = tf.reshape(input, shape=(cfg.batch_size, input.shape[1], 1, input.shape[-2].value, 1))
                # print("self.input2", self.input)
...
def routing(input, b_IJ, num_outputs=10, num_dims=16):
	...
	动态路由算法:
		初始化b_IJ=0
		迭代r次：
			step_1:c_i←softmax(bi)；
			step_2:s_j←∑i c_ij u^j|i；
			step_3:v_j←squash(s_j)；
			step_4:b_IJ←b_IJ+<u^j|i,v_j>
		返回vj。
     '''
    ...
    with tf.variable_scope('routing1',reuse=tf.AUTO_REUSE):
